...
def tt_scaling(X_tt, Z_tt):
    norm_x = np.sqrt(tt_inner_prod(X_tt, X_tt))
    norm_z = np.sqrt(tt_inner_prod(Z_tt, Z_tt))
    return np.sqrt(norm_x / norm_z)
...
def tt_ipm(
    ...
    Y_tt = [np.zeros((1, 4, 1)) for _ in range(2*dim)]
    T_tt = None
    if active_ineq:
