...
def _tt_line_search(
    ...
    x_step_size, status.eigen_x0 = tt_max_generalised_eigen(X_tt, Delta_X_tt, x0=status.eigen_x0, tol=1e-3*status.op_tol, verbose=status.verbose)
    z_step_size, status.eigen_z0 = tt_max_generalised_eigen(Z_tt, Delta_Z_tt, x0=status.eigen_z0, tol=1e-3*status.op_tol, verbose=status.verbose)
...
def _ineq_step_size(A_tt, Delta_tt, e_tt, status):
    ...
    e_tt, _ = tt_min_eig(tt_diag_op(sum_tt, status.eps), x0=e_tt, tol=1e-3*status.op_tol, verbose=status.verbose)
