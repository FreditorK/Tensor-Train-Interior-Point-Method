...
def _get_eq_mat_vec(XAX_k, block_A_k, XAX_kp1, x_shape, inv_I):
    ...
    def mat_vec(x_core):
        ...
        result[1] = L_Z(XAX_k[(2, 1)], block_A_k[(2, 1)], XAX_kp1[(2, 1)], x_core[1]).__isub__(L_X( XAX_k[(2, 2)], block_A_k[(2, 2)], XAX_kp1[(2, 2)], inv_I*mL_adj(XAX_k[(0, 1)], block_A_k[(0, 1)], XAX_kp1[(0, 1)], x_core[0])))
...
def tt_infeasible_newton_system(
    ...
    if status.with_ineq:
        ...
        if not status.is_central or status.is_last_iter:
            rhs[3] = tt_rank_reduce(tt_reshape(tt_scale(-1, tt_fast_hadammard(masked_X_tt, T_tt, status.eps)), (4, )), eps=status.op_tol, rank_weighted_error=True)
