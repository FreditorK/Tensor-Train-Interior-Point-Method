...
def tt_max_eig(matrix_tt, nswp=10, x0=None, eps=1e-10, verbose=False):
    return _tt_eig(matrix_tt, min_or_max="LA", nswp=nswp, x0=x0, eps=eps, verbose=verbose)


def tt_min_eig(matrix_tt, nswp=10, x0=None, eps=1e-10, verbose=False):
    return _tt_eig(matrix_tt, min_or_max="SA", nswp=nswp, x0=x0, eps=eps, verbose=verbose)

def _tt_eig(A, min_or_max, nswp, x0, eps, verbose):
    if verbose:
        print(f"Starting Eigen solve with:\n \t {eps} \n \t sweeps: {nswp}")
        t0 = time.time()
    normalisation = np.sqrt(tt_inner_prod(A, A))
    A = tt_scale(1/normalisation, A)

    if x0 == None:
        x_cores = tt_normalise([np.ones_like(c[:, :, 0]) for c in A])
    else:
        x_cores = x0.copy()
    ...
    for swp in range(nswp):
        ...
        max_res = 0
        for k in range(d):
            B = einsum("lsr,smnS,LSR->lmLrnR", XAX[k], A[k], XAX[k + 1], optimize=True).reshape(rx[k] * N[k] * rx[k + 1], rx[k] * N[k] * rx[k + 1])
    ...
    return normalisation*tt_inner_prod(x_cores, tt_fast_matrix_vec_mul(A, x_cores, eps)), x_cores, max_res
