...
def tt_eig(A, nswp=22, x0=None, eps=1e-10, kickrank=4, kick2=0, trunc_norm='res', verbose=False, band_diagonal=-1):

    return _amen_solve_eig(A, nswp, x0, eps, kickrank, kick2, trunc_norm, verbose, band_diagonal)


def _amen_solve_eig(A, nswp=22, x0=None, eps=1e-10, kickrank=4, kick2=0, trunc_norm='res', verbose=False, band_diagonal=-1):

    dtype = A[0].dtype
    damp = 2

    if x0 == None:
        x = [np.ones_like(c[2], dtype=dtype) for c in A]
    else:
        x = x0
    ...
    for swp in range(nswp):
        ...
        for k in range(d):
            ...
            eig_val, solution_now = scip.sparse.linalg.eigs(B, k=1)
            rhs = eig_val*solution_now

            res_old = np.linalg.norm(B @ previous_solution-rhs)
