...
def build_graphm_constraints(n):
    ...
    eq_rows.append(scs_row_from_entries(size, [(last, last, 1.0)]))
    eq_rhs.append(1.0)
    ...
    for i in range(n):
        for j in range(n):
            q = q_idx(i, j)
            p = p_idx(j, i)
            eq_rows.append(scs_row_from_entries(size, [(q, q, 1.0), (p, last, -1.0)]))
            eq_rhs.append(0.0)
    ...
    for row in range(n):
        for col in range(n):
            ineq_rows.append(scs_row_from_entries(size, [(p_idx(row, col), last, 1.0)]))
            ineq_rhs.append(0.0)

    for r in range(n2):
        for c in range(n2):
            ineq_rows.append(scs_row_from_entries(size, [(r, c, 1.0)]))
            ineq_rhs.append(0.0)
...
if __name__ == "__main__":
    ...
    for s_i, seed in enumerate(config["seeds"]):
        ...
        feasibility_errors[s_i] = info.get("res_pri", np.nan) ** 2
        dual_feasibility_errors[s_i] = info.get("res_dual", np.nan) ** 2
        num_iters[s_i] = info.get("iter", 0)
