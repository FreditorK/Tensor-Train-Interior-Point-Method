...
if __name__ == "__main__":
    ...
    for s_i, seed in enumerate(config["seeds"]):
        for attempt in range(1):  # At most two tries: original and one new random seed
            ...
            constraints += [P >= 0, cp.sum(P, axis=0) == 1, cp.sum(P, axis=1) == 1]
            for i in range(n):
                for j in range(n):
            ...
            try:
                if args.track_mem:
                    def wrapper():
                        try:
                            ...
                            _ = prob.solve(solver=cp.SDPA, epsilonDash=1e-6 / n, epsilonStar=1e-5 / n, verbose=True, omegaStar=100.0, betaStar=0.5, gammaStar=0.7) # needed to reduce for stability
                        except:
                            pass
                        return prob
                    ...
                    try:
                        prob = cp.Problem(objective, constraints)
                        _ = prob.solve(solver=cp.SDPA, epsilonDash=1e-6 / n, epsilonStar=1e-5 / n, verbose=True, omegaStar=100.0, betaStar=0.5, gammaStar=0.9) # needed to reduce for stability
                    except:
                        pass
                    X_val = QP_mat.value
                ...
                feasibility_errors[s_i] = 0  # Placeholder, customize if needed
                dual_feasibility_errors[s_i] = 0  # Placeholder, customize if needed
