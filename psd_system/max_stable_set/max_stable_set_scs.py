...
if __name__ == "__main__":
    ...
    for s_i, seed in enumerate(config["seeds"]):
        for attempt in range(1):
            ...
            eq_rows = [scs_row_from_entries(n, [entry]) for entry in mask_entries]
            eq_rows.append(scs_row_from_entries(n, [(i, i, 1.0) for i in range(n)]))
            ...
            try:
                if args.track_mem:
                    def wrapper():
                        return solve_scs_psd_max(J, eq_rows, eq_rhs, eps=1e-5 / config["dim"], verbose=True)

                    res_mem, sol = memory_usage(proc=wrapper, max_usage=True, retval=True, include_children=True)
                    memory[s_i] = res_mem - start_mem
                else:
                    sol = solve_scs_psd_max(J, eq_rows, eq_rhs, eps=1e-5 / config["dim"], verbose=True)
                break
