...
#PBS -N sdpa_stable_set
#PBS -o sdpa_max_stable_set.out
#PBS -e sdpa_max_stable_set.err
...
echo "==== Max Stable Set SDPA Batch Run Started at $(date) ===="
...
for dim in $(seq $START_DIM $END_DIM); do
    ...
    timeout "$CURRENT_TIMEOUT" python max_stable_set_sdpa.py --config "$CONFIG" --track_mem
    status=$?
