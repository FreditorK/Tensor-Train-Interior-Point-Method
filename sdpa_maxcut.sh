...
#PBS -N sdpa_maxcut
#PBS -o sdpa_maxcut.out
#PBS -e sdpa_maxcut.err
...
echo "==== Maxcut SDPA Batch Run Started at $(date) ===="
...
for dim in $(seq $START_DIM $END_DIM); do
    ...
    timeout "$CURRENT_TIMEOUT" python maxcut_sdpa.py --config "$CONFIG" --track_mem
    status=$?
