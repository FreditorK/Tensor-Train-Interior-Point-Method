...
#PBS -N skcgal_maxcut
#PBS -o skcgal_maxcut.out
#PBS -e skcgal_maxcut.err
...
echo "==== Maxcut Sketchy Cgal Batch Run Started at $(date) ===="
...
for dim in $(seq $START_DIM $END_DIM); do
    ...
    timeout "$CURRENT_TIMEOUT" python maxcut_sketch_cgal.py --config "$CONFIG" --track_mem
    status=$?
