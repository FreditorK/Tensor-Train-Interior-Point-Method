...
usage() {
    echo "Usage: bash scs.sh <problem> <start_dim> <end_dim> <rank> [--track_mem]"
}
...
echo "Running: ${PROBLEM}_scs (rank=${RANK})"
BASE_TIMEOUT=6400  # 2h
mkdir -p results
LOGFILE="results/scs_${PROBLEM}_${START_DIM}_${END_DIM}_${RANK}.txt"
rm -f "$LOGFILE"
...
echo "==== ${PROBLEM} SCS Batch Run Started at $(date) ===="
...
for dim in $(seq $START_DIM $END_DIM); do
    ...
    cmd=(python "${PROBLEM}_scs.py" --config "$CONFIG" --rank "$RANK")
    if [ -n "$TRACK_MEM" ]; then
        cmd+=("$TRACK_MEM")
    fi
