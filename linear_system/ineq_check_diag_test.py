...
matrix_tt_1 = tt_scale(0.009, tt_add(tt_one_matrix(4), [np.array([[1, 0], [0, 0]]).reshape(1, 2, 2, 1)] + tt_scale(0.21, tt_identity(3))))
matrix_tt_1 = tt_diag(tt_split_bonds(matrix_tt_1))
a = tt_random_gaussian([1, 1], shape=(2, 2))
matrix_tt_2 = tt_diag(tt_split_bonds(tt_add(tt_scale(0.001, tt_one_matrix(4)), [np.array([[1, 0], [0, 0]]).reshape(1, 2, 2, 1)] + a)))

step_size, _ = tt_ineq_optimal_step_size(matrix_tt_1, matrix_tt_2, 1e-8, verbose=True)
