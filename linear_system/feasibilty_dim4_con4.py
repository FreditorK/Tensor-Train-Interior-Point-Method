...
t1 = time.time()
tt_train_b = tt_matrix_vec_mul(gram_tensor, tt_scale(3 * np.random.rand(), tt_random_binary([
...
X_sol = tt_matrix_vec_mul(tt_transpose(tensor_matrix), index_sol)
print("Ranks of solution: ", [c.shape for c in X_sol])
residual = tt_add(tt_matrix_vec_mul(tensor_matrix, X_sol), tt_scale(-1, tt_train_b))
