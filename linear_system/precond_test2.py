import time
import numpy as np
import scipy.sparse as sp
import scipy.sparse.linalg as spla
from scipy.linalg import block_diag

# --- Problem setup ---
np.random.seed(42)
n1, n2 = 50, 50
block_size = 50

B1 = np.random.rand(20,20)
B2 = np.random.rand(20,20)
B3 = np.random.rand(10,10)
K_y_dense = block_diag(B1 + B1.T, B2 + B2.T, B3 + B3.T)
K_y = sp.csr_matrix(K_y_dense)

Z_dense = np.random.rand(n2, n2)
Z_dense = Z_dense + Z_dense.T + 1e-8*np.eye(n2)
Z = sp.csr_matrix(Z_dense)

L = sp.random(n1, n2, density=0.1, format='csr')
P = sp.random(n2, n1, density=0.1, format='csr')

A = sp.bmat([[K_y, L],
             [P, Z]], format='csr')
b = np.random.rand(n1+n2)

# --- Helper: block-diagonal inverse approximation ---
def block_diagonal_inverse(block, block_size, reg=1e-8):
    n = block.shape[0]
    inv_blocks = []
    for start in range(0, n, block_size):
        end = min(start+block_size, n)
        sub = block[start:end, start:end].toarray() + reg*np.eye(end-start)
        inv_blocks.append(np.linalg.pinv(sub))
    return inv_blocks

def solve_block(block_inv, x):
    n = len(x)
    y = np.zeros_like(x, dtype=np.float64)
    for i, inv in enumerate(block_inv):
        start = i*block_size
        end = min(start+block_size, n)
        y[start:end] = inv @ x[start:end]
    return y

# Precompute block-diagonal inverses for Z
Z_inv_blocks = block_diagonal_inverse(Z, block_size)

# --- Schur complement matvec ---
def schur_matvec(v):
    Pv = P @ v
    Zinv_Pv = solve_block(Z_inv_blocks, Pv)
    return K_y @ v - L @ Zinv_Pv

S_op = spla.LinearOperator((n1,n1), matvec=schur_matvec)

def apply_Sinv(v):
    x, info = spla.lgmres(S_op, v, rtol=1e-3, maxiter=10)
    return x

# --- Block-diagonal Schur preconditioner ---
def block_diag_schur_prec(x):
    x1, x2 = x[:n1], x[n1:]
    y1 = apply_Sinv(x1)
    y2 = solve_block(Z_inv_blocks, x2)
    return np.concatenate([y1, y2])

M = spla.LinearOperator(A.shape, matvec=block_diag_schur_prec)

# --- GMRES with Schur-based preconditioner ---
callback_counter = {'iter':0}
def cb(xk): callback_counter.update({'iter': callback_counter['iter']+1})
t0 = time.time()
x_prec, info_prec = spla.lgmres(A, b, M=M, rtol=1e-6, inner_m=50, callback=cb)
time_prec = time.time() - t0
iters_prec = callback_counter['iter']

# --- GMRES without preconditioner ---
callback_counter_no = {'iter':0}
def cb2(xk): callback_counter_no.update({'iter': callback_counter_no['iter']+1})
t0 = time.time()
x_no, info_no = spla.lgmres(A, b, rtol=1e-6, inner_m=50, callback=cb2)
time_no = time.time() - t0
iters_no = callback_counter_no['iter']

# --- Results ---
print("GMRES without preconditioner iterations:", iters_no, "time:", time_no, "s")
print("GMRES with block-diagonal Schur preconditioner iterations:", iters_prec, "time:", time_prec, "s")
print("Preconditioned solution error:", np.linalg.norm(A.dot(x_prec)-b))
print("Unpreconditioned solution error:", np.linalg.norm(A.dot(x_no)-b))







