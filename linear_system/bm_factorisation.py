...
A_psd = [np.eye(2).reshape(1, 2, 2, 1)] + tt_rank_reduce(tt_mat_mat_mul(A, tt_transpose(A)))
t0 = time.time()
B_psd, err = tt_burer_monteiro_factorisation(A_psd, is_block=True, num_swps=15, max_iter=15, verbose=True)
t1 = time.time()
